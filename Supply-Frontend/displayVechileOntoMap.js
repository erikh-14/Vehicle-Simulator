...
//function to retrieve vehicle information from vsim
function getVehicleLocationFromVSIM(){
    ...
    var url = 'https://supply.team12.softwareengineeringii.com/api/backend?vehicle-all=1'; //the path needed to get into the database 
    ...
    request.onload = function(){
        ...
        if(request.status >= 200 && request.status < 400){
            ...
            //Display all vehicle onto the map: 
                 map.on('load', function() {
                    ...
                    vehicleInfo.forEach(element => {
                        ...
                        console.log(element)     
                        vehicleName_testing = element['vehicle_name'];
                        vehicleLocation_testing = element['vehicle_position'];
                        vehicle_id = element['_id'];
                        // console.log(vehicleName_testing)
                        // console.log(vehicleLocation_testing)
                     console.log(vehicle_id)
                    map.addSource(vehicle_id, {
                        ...
                        'features': [
                        {
                        // feature for vehicle 1
                        'type': 'Feature',
                        'geometry': {
                        'type': 'Point',
                        'coordinates': vehicleLocation_testing
                        },
                        'properties': {
                        'title': vehicleName_testing,
                        'icon': 'car'
                        }
                    }
                    ]
...
//Function to display warehouse information from datbase onto the fleetmanger map
function displayWarehouseLocationOntoMap(){
    var warehouseLocation = [];
    var warehouseLong = 0.0; 
    var warehouseLat = 0.0; 
    var warehouseName = "";
    // var warehouse_coordinates_array = [];
    var warehouse_name_02 = "";
    var warehouse_long_02 = 0.0;
    var warehoues_lat_02 = 0.0;
    var warehouse_name_03 = "";
    var warehouse_long_03 = 0.0;
    var warehoues_lat_03 = 0.0;
    var warehouse_name_04 = "";
    var warehouse_long_04 = 0.0;
    var warehoues_lat_04 = 0.0;
    var warehouseLocation_02 = [];
    var warehouseLocation_03 = [];
    var warehouseLocation_04 = [];
    ...
    //SUBJECT TO CHANGES:
    var url = 'https://supply.team12.softwareengineeringii.com/api/backend/fetchfleet'; //the path needed to get into the database 
    //making a bridge to the web server
    var request = new XMLHttpRequest();
    //initializing the request to the web server through a GET request 
    ...
    //Determining the status of the request. 
    request.onload = function(){
        console.log('displayWarehouseLocationOntoMap() is called!');
        //Using JSON to parse API response:
        var wareHouseInfo = JSON.parse(this.response);
        //Getting the length of the json object:
        var jsonLength = Object.keys(wareHouseInfo).length;
        //Good response: 
        if(request.status >= 200 && request.status < 400){
            //WHAT IS IN THE JSON RESPONSE:
            // console.log(wareHouseInfo);
            // console.log(wareHouseInfo['warehouses']);
            //Warehouse location: 
            // console.log(wareHouseInfo['warehouses'][0]['coordinates'][0])
            // console.log(wareHouseInfo['warehouses'][1]['coordinates'][1])
            // console.log(wareHouseInfo['warehouses'][0]['warehouse_name'])


            //printing out each warehouse object in the response back from the server by looping through the response:
            // for(i = 0; i < (jsonLength - 1); i++){
            //     console.log(wareHouseInfo['warehouses'][i]['coordinates'][0])
            //     console.log(wareHouseInfo['warehouses'][i]['coordinates'][1])
            //     console.log(wareHouseInfo['warehouses'][i]['warehouse_name'])

            //     //Warehouse information is extracted from the response and store locally onto the JS file:
            //     warehouseName = wareHouseInfo['warehouses'][i]['warehouse_name'];
            //     warehouseLong = wareHouseInfo['warehouses'][i]['coordinates'][0];
            //     warehouseLat = wareHouseInfo['warehouses'][i]['coordinates'][1]; 
            //     warehouseLocation_Loop = [warehouseLong, warehouseLat];
            //     //appending each warehouse location into an array:
            //     // warehouse_coordinates_array.push(warehouseLocation_Loop);
            //     //Viewing the contents of an array:
            //     console.log("The contents with in an array of coordinates: ")
            //     // console.log(warehouse_coordinates_array)
            ...
            //Storing warehouses infrormation into the JS file and display them onto the map: 
            //variables initialization: 
            warehouseName = wareHouseInfo['warehouses'][0]['warehouse_name'];
            warehouseLong = wareHouseInfo['warehouses'][0]['coordinates'][0];
            warehouseLat = wareHouseInfo['warehouses'][0]['coordinates'][1]; 
            warehouseLocation = [warehouseLong, warehouseLat];

            //warehouse edwards information:
            warehouse_name_02 = wareHouseInfo['warehouses'][1]['warehouse_name'];
            warehouse_long_02 = wareHouseInfo['warehouses'][1]['coordinates'][0];
            warehoues_lat_02 = wareHouseInfo['warehouses'][1]['coordinates'][1]; 
            warehouseLocation_02 = [warehouse_long_02, warehoues_lat_02];

            //warehouse tang information:
            warehouse_name_03 = wareHouseInfo['warehouses'][2]['warehouse_name'];
            warehouse_long_03 = wareHouseInfo['warehouses'][2]['coordinates'][0];
            warehoues_lat_03 = wareHouseInfo['warehouses'][2]['coordinates'][1];
            warehouseLocation_03 = [warehouse_long_03, warehoues_lat_03]

            //warehouse lakeways information: 
            warehouse_name_04 = wareHouseInfo['warehouses'][3]['warehouse_name'];
            warehouse_long_04 = wareHouseInfo['warehouses'][3]['coordinates'][0];
            warehoues_lat_04 = wareHouseInfo['warehouses'][3]['coordinates'][1]
            warehouseLocation_04 = [warehouse_long_04, warehoues_lat_04]
            ...
            map.on('load', function() {
                ...
                map.addSource('warehouse-longhorn', {
                    'type': 'geojson',
                    'data': {
                    'type': 'FeatureCollection',
                    'features': [
                    ...
                    },
                    {
                        ...
                        'geometry': {
                        'type': 'Point',
                        'coordinates': warehouseLocation_02
                        },
                        'properties': {
                        'title': warehouse_name_02,
                        'icon': 'harbor'
                        }
                    },

                    {
                        ...
                        'geometry': {
                        'type': 'Point',
                        'coordinates': warehouseLocation_03
                        },
                        'properties': {
                        'title': warehouse_name_03,
                        'icon': 'harbor'
                        }
                    },
                    ...
                    //Adding a commment.
                    {
                        ...
                        'geometry': {
                        'type': 'Point',
                        'coordinates': warehouseLocation_04
                        },
                        'properties': {
                        'title': warehouse_name_04,
                        'icon': 'harbor'
                        }
                    },
                    ]
                ...
                map.addLayer({
                    'id': 'warehouse-longhorn',
                    'type': 'symbol',
                    'source': 'warehouse-longhorn',
                    'layout': {
                    // get the icon name from the source's "icon" property
                    // concatenate the name to get an icon from the style's sprite sheet
                    'icon-image': ['concat', ['get', 'icon'], '-15'],
    ...
    //Specifying the right fleet to pull information from:
    var fleetData = {"fleet_id": "5e8ff9fe90a566d2e2cc8616"};
